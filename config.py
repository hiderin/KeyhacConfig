...
# 追加したい機能のメモ
#-------------------------------------------------------------------------------
# ・クラスをテキスト編集クラスとそうでないクラスに分ける
# ・select_lineで行末の改行が含まれるかどうかのクラス分け
# ・挿入モードでLC-pで貼り付け
# ・M電卓を常に手前に表示した時のLC-0,LC-9のWindow切り替えの制御
# ・line単位でのコピー切り取り後の貼り付け処理が不十分
# ・キーボードマクロをiniファイルに登録してマクロを流用する
# ・リーピート回数のMAX値を決める、コマンドから可変にする。
# ・左右のCtrlとShiftをうまく使い分けてC-SpaceやS-Spaceを復活させる
# ・LC-Shiftはvimmodeに戻すのみでLC-Shift2回にESCにする。
################################################################################
